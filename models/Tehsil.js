...
module.exports = function (sequelize, DataTypes) {
    var myModel = sequelize.define("Tehsil",
            {
                tehsil_name: {
                    ...
                    validate: {
                        ...
                        isUnique: function (value, next) {
                            var self = this;
                            myModel.find({
                                where: {tehsil_name: value},
                                attributes: ['id']
                            })
                                    .then(function (data) {
                                        // reject if a different user wants to use the same email
                                        if (data && Number(self.id) !== data.id) {
                                            return next(i18n_Validation.__('Please_Enter', 'Unique Tehsil Name'));
                                        }
                                        return next();
                                    })
                                    .catch(function (err) {
                ...
                tehsil_code: {
                    ...
                    validate: {
                        ...
                        isUnique: function (value, next) {
                            var self = this;
                            myModel.find({
                                where: {tehsil_code: value},
                                attributes: ['id']
                            })
                                    .then(function (data) {
                                        // reject if a different user wants to use the same email
                                        if (data && Number(self.id) !== data.id) {
                                            return next(i18n_Validation.__('Please_Enter', 'Unique Tehsil Code'));
                                        }
                                        return next();
                                    })
                                    .catch(function (err) {
            ...
            },
    {
        ...
        classMethods: {
            ...
            getAllValuesPaging: function (req, res) {
                ...
                var mymodelDistrictBelongsTo = myModel.belongsTo(sequelize.models.District, {foreignKey: 'district_id'})
                myModel.findAndCountAll({
                    where: req.where,
                    offset: req.offset,
                    limit: req.limit,
                    order: 'id DESC',
                    include:[mymodelStateBelongsTo,mymodelCountryBelongsTo,mymodelDivisionBelongsTo,mymodelDistrictBelongsTo]
        ...
        hooks: {
            beforeCreate: function (instance) {
                instance.set('tehsil_slug', slugify(instance.get('tehsil_name')));
            }
        }
