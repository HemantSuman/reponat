...
module.exports = function (sequelize, DataTypes) {
    var myModel = sequelize.define("State",
            {
                state_name: {
                                ...
                                attributes: ['id']
                                    ...
                                    .then(function (data) {
                                        ...
                                        if (data && Number(self.id) !== data.id) {
                                            return next(i18n_Validation.__('Please_Enter', 'Unique State Name'));
                                        }
                                        return next();
                                    })
                                    .catch(function (err) {
                ...
                state_code: {
                    ...
                    validate: {
                        ...
                        isUnique: function (value, next) {
                            var self = this;
                            myModel.find({
                                where: {state_code: value},
                                attributes: ['id']
                            })
                                    .then(function (data) {
                                        // reject if a different user wants to use the same email
                                        if (data && Number(self.id) !== data.id) {
                                            return next(i18n_Validation.__('Please_Enter', 'Unique State Code'));
                                        }
                                        return next();
                                    })
                                    .catch(function (err) {
        ...
        classMethods: {
            ...
            getAllValuesPaging: function (req, res) {
                ...
                myModel.findAndCountAll({
                    ...
                    limit: req.limit,
                    order: 'id DESC',
                    include:[mymodelCountryBelongsTo]
            ...
            saveAllValues: function (values, res) {
                ...
                if (values.body.is_business_available === 'on') {
                    values.body.is_business_available = 1;
                } else {
                    values.body.is_business_available = 0;
                }
...
//                instance.set('state_slug', slugify(instance.get('state_name')));
            }
        }
