...
module.exports = function (sequelize, DataTypes) {
    var myModel = sequelize.define("BusinessCategory",
            {
                business_category_name: {
                    ...
                    validate: {
                        ...
                        isUnique: function (value, next) {
                            var self = this;
                            myModel.find({
                                where: {business_category_name: value},
                                attributes: ['id']
                            })
                                    .then(function (data) {
                                        // reject if a different user wants to use the same email
                                        if (data && Number(self.id) !== data.id) {
                                            return next(i18n_Validation.__('Please_Enter', 'Unique BusinessCategory Name'));
                                        }
                                        return next();
                                    })
                                    .catch(function (err) {
...
//                instance.set('business_category_slug', slugify(instance.get('business_category_name')));
            }
        }
