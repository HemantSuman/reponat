//Product manager
var arr = [];
$(document).on('submit', '#class', (function (event, state) {
    $('.errorMsg').remove();
    $("#class .form-group input, #class .form-group select").removeClass('errMsg');
...
$(document).on('submit', '#post', (function (event, state) {
    $('.errorMsg').remove();
    $("#class .form-group input, #post .form-group select").removeClass('errMsg');
    event.preventDefault();
    var form = $('#post')[0];
    var formData = new FormData(form);
    formData.files = arr;
