...
var extraVar = [];
var viewDirectory = 'revised_fees';
var modelName = 'RevisedFees';
var titleName = 'Revised Fees';
                ...
                if (errors.length == 0) {
                    ...
                    models[modelName].checkAlreadyExist(req, function (checkStatus) {
                        if(checkStatus.status) {
                            ...
                            res.status(201).send({status: false, message: 'An entry already exist for this duration.', data: []});
                        }
                    });                    
                    ...
                    models[modelName].checkAlreadyExist(req, function (checkStatus) {
                        if(checkStatus.status) {
                            models[modelName].updateAllValues(req, function (results) {
                                if (results.status) {
                                    req.flash('type_messages', 'success');
                                    req.flash('messages', i18n_Validation.__('updated_msg'));
                                    res.status(200).send({status: true, url: '/admin/' + viewDirectory + '/' + req.body.product_id});
                                } else {
                                    res.status(400).send({status: false, msg: ' saved d failed', data: results.errors});
                                }
