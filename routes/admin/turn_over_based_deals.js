...
var extraVar = [];
var viewDirectory = 'turn_over_based_deals';
var modelName = 'TurnOverBasedDeal';
var titleName = 'Turn Over Based Deal';
...
router.get('/:product_id', adminAuth.isAllow, adminAuth.productIdChk, function (req, res, next) {
    ...
    models[modelName].getAllValuesPaging(req, function (results) {
        ...
        res.render('admin/' + viewDirectory + '/index', {results: results, extraVar: extraVar, layout: 'admin/layout/layout', serchData: req.query});
    });
...
router.get('/edit/:id/:product_id', adminAuth.isAllow, adminAuth.productIdChk, function (req, res, next) {
    setProductType(req.params.product_id)
...
router.post('/delete/:id/:product_id', adminAuth.isAllow, adminAuth.productIdChk, function (req, res, next) {
    var id = req.params.id;
    setProductType(req.params.product_id)
    req.where = {'id': id};
                ...
                if (errors.length == 0) {
                    ...
                    models[modelName].checkAlreadyExist(req, function (checkStatus) {
                        if (checkStatus.status) {
                            ...
                            res.status(201).send({status: false, message: 'An entry already exist for this duration or the range.', data: []});
                        }
                    });
