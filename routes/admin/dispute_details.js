...
var extraVar = [];
var viewDirectory = 'dispute_details';
var modelName = 'DisputeDetail';
var titleName = 'Property Disputes';
                ...
                if (errors.length == 0) {
                    ...
                    models[modelName].saveAllValues(req, function (results) {
                        if (results.status) {
                            ...
                            res.status(200).send({status: true, url: '/pages/property-dispute', msg: 'Thank you for submitting your details, our team will contact you soon.'});
                        } else {
...
router.get('/download_pdf/:id', function (req, res, next) {
    ...
    models[modelName].getFirstValues(req, function (results) {
        ...
        var options = {method: 'GET',
            url: fullUrl + '/admin/dispute-details/pdf/' + req.params.id,
            headers:
                    {
        ...
        var fileName = results.dataValues.name + '_' + results.dataValues.number + '.pdf';
        ...
        request(options, function (error, response, body) {
            ...
            var options = {
                ...
                "footer": {
                    "height": "10mm",
                    "contents": {
                        default: '<p style="height:30px;"></p><span style="display: inline-block;text-align: center;width: 100%;"><span style="color: #444;text-align:center;">Page {{page}} of </span><span>{{pages}}</span>  Dated ' + req.app.locals.site.momentObj().format('DD-MM-YYYY') + '</span>',
                    }
                }
            };
            html = body;
